...
/*				ff.setAttribute('class','smoke-base'); */
	...
	build: function (e, f) {
		...
		if (f.type !== 'signal'){
			buttons = '<div class="dialog-buttons text-center">';
			if (f.type === 'alert'){
				buttons +=
					'<button class="btn btn-danger" id="alert-ok-'+f.newid+'">'+ok+'</button>';
			} else if (f.type === 'prompt' || f.type === 'confirm'){
				if (f.params.reverseButtons) {
					buttons +=
						'<button class="btn btn-danger" id="'+f.type+'-ok-'+f.newid+'">'+ok+'</button>' +
						'<button class="btn" id="'+f.type+'-cancel-'+f.newid+'" class="cancel" style="margin-left:10px;">'+cancel+'</button>';				
				} else {
					buttons +=
						'<button class="btn btn-primary" id="'+f.type+'-ok-'+f.newid+'" >'+ok+'</button>'+
						'<button class="btn btn-danger" id="'+f.type+'-cancel-'+f.newid+'" class="cancel" style="margin-left:10px;">'+cancel+'</button>';
		...
		// 获取main的div元素 
		// 通过窗口宽高和div宽高计算位置 
		var main_left = (windowScreen.clientWidth -300)/2 + "px"; 
		var main_top = (windowScreen.clientHeight - 200)/2 + "px"; 
			box = '<div id="smoke-bg-'+f.newid+'" class="smokebg"></div>'+
			'<div class="dialog'+classname+'">'+
				'<div class="gmTips_warp" style="display: block;padding-bottom:5px;overflow:hidden;">'+
		'<div class="gmTips_hd">'+
			'<b class="close gmTips_close gClose" id="gClose'+f.newid+'"></b>'+
			'<span class="gmTips_title">温馨提示</span>'+
		'</div>'+
    		'<div class="gmTips_bd">'+
			'<div class="pop_del_box">'+e+'</div>'+
			'<div class="txtCen mt10">'+
			prompt+
			buttons+
			'</div>'+
    		'</div>'+
...
//		);
	...
	finishbuildAlert: function (e, f, box)
	{
		...
		smoke.listen(
				document.getElementById('gClose' + f.newid),
				"click", 
				function () 
				{
					smoke.destroy(f.type, f.newid);
