...
//						dataPanel.append($('<div class="row"/>').html(horStr));
			...
			createProgressBar: function(days, bID, cls, desc, label) {
				...
				var num = Number(label.split(':')[1].substring(0,label.split(':')[1].length-1));
				var labelDis = label.split(':')[0]+":"+label.split(':')[1];
				var endDay = label.split(':')[2];
				...
				var strToday = today.getFullYear() + '-' + month + '-' + today.getDate();
				strToday = strToday.replace(/-/g,'');
				endDay = endDay.replace(/-/g,'');
				var mulite = Number(strToday) - Number(endDay);
				
				//画绿色的线条
				if(barId.indexOf("_green") != -1){
					
					var bar = $("<div "+ barId +" class='bar'></div>")
							.addClass( cls )
							.css({
								width: cellWidth * (days+2) * num / 100
					...
					//画底色的线条
					cls = 'ganttOrange';
					var width = cellWidth * (days+2);
					if(num == 100){
						cls = 'ganttGreen';
					}else{
						if(mulite > 0){
							cls = 'ganttRed';
							width = width + cellWidth * 3 * mulite;
						}
					}
				...
				if (desc)
				{
					bar
					  .mouseover(function(e){
					  	var hint = $("<div class='fn-gantt-hint' />").html(labelDis);
					  	$("body").append(hint);
						...
						$.each(entry.values, function(j, day) {
							...
							switch (settings.scale)
							{
								case 'hours':
									...
									_bar_1 = core.createProgressBar(
											dl,
											day.id ? day.id+'_green' : '',
											'ganttGreen',
											day.desc ? day.desc : '',
											day.label ? day.label : ''
										);
