...
define(function (require) {
    ...
    function point(zr, effectList, shape, zlevel) {
        ...
        if (shape.type !== 'images') {
            ...
            if (canvasSupported) {  // 提高性能，换成images
                effectShape.style.images = zr.shapeToimages(
                    effectShape, 
                    effectShape.style.width + shadowBlur * 2 + 2, 
                    effectShape.style.height + shadowBlur * 2 + 2
                ).style.images;
                
                effectShape = new imagesShape({
        ...
        var devicePixelRatio = shape.type !== 'images' ? (window.devicePixelRatio || 1) : 1;
    ...
    function line(zr, effectList, shape, zlevel, isLarge) {
        ...
        var offset = 0;
        if (canvasSupported && ! isLarge) {  // 提高性能，换成images
            var zlevel = effectShape.zlevel;
            effectShape = zr.shapeToimages(
                effectShape,
                (size + shadowBlur) * 2,
                (size + shadowBlur) * 2
            );
